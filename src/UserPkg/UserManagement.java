...
public class UserManagement {
	...
	/**
	 * Checks if the password introduced by a user attempting to register
	 * respects the requirements.
	 * 
	 * @param password
	 *            the password to be checked
	 * @return boolean the result of the password check (true = password meets
	 *         requirements; false = password does not meet requirements)
	 */
	 ...
	 * 
	 * @param userNumber
	 *            integer used to identify which one of the two players to
	 *            instantiate (i.e. left or right) username the username of the
	 *            user attempting to log in password the password of the user
	 *            attempting to log in
	 * @return UserLoginStatus represents the result of the login (success;
	 *         wrong password; username not found; user already logged in)
	 */
	public static UserLoginStatus login(int userNumber, String username,
			String password) {
		...
		/*
		 * Iterate through the list of users to check if the username-password
		 * combination entered by the user exists
		 */
		for (User user : users) {
		...
		/*
		 * If this point is reached, it means that the username was not found in
		 * the list of registered users, therefore the appropriate username not
	 ...
	 * 
	 * @param username
	 *            the username of choice password the password or the user
	 * @return UserRegistrationStatus represents the result of the register
	 *         (success; username taken; password requirements not met; error in
	 *         creating user)
	 */
	public static UserRegistrationStatus registerUser(String username,
			String password) {

		/*
		 * The list of registered users is first read from the user_data.csv
		 * file in order to ensure that the username is not taken
		 */
		ArrayList<User> users = new ArrayList<User>();
		users = (ArrayList<User>) CSVHandler.read("csv/user_data.csv");

		/*
		 * If, when iterating through the list of existing users, the username
		 * introduced in the GUI is found, return the appropriate UsernameTaken
		 * status
		 */
		for (User user : users) {
			if ((user.getUsername()).equals(username)) {
		...
		/*
		 * If both checks above pass, create the user with the introduced
		 * username and password and add him/her to the previously read list
		 */
		...
		/*
		 * If this point is reached it means that the recently created user was
		 * not found in the user_data.csv file
		 */
